...
CONFIG_FILE = Path(__file__).with_name("case_Toluene_dualORC_mass_flow_macchi_astolfi.yaml")
...
def _modify_yaml_dual(yaml_text, hp_stages, lp_stages):
    ...
    Expects the YAML to have hp_expander and lp_expander sections
    with n_stages fields.
    """
    # Replace hp_expander n_stages (first occurrence)
    yaml_text = re.sub(
    ...
    # Replace lp_expander n_stages (first occurrence)
    yaml_text = re.sub(
...
def run_parametric(config_files, hp_stages_list, lp_stages_list, output_dir):
    ...
    print("=" * 76)
    print("  PARAMETRIC STUDY — Dual-Pressure Macchi & Astolfi")
    print("=" * 76)
