...
plot_pcond_sweep_recup.py automatically.
...
-----
  ...
  4. Re-run plot_pcond_sweep_recup.py to see the final figures

The script updates LABEL_OFFSETS inside plot_pcond_sweep_recup.py in the
same folder.  A backup is written as plot_pcond_sweep_recup.py.bak first.
"""
...
# ── CONFIGURATION ─────────────────────────────────────────────────────────────
FIGURE = "fig1_recup_subcritical"  # "fig1_recup_subcritical" | "fig2_recup_transcritical" | "fig3_recup_combined"
EXCEL_FILE = "pcond_vs_efficiency_recup.xlsx"
PLOT_SCRIPT = "plot_pcond_sweep_recup.py"  # file whose LABEL_OFFSETS gets updated
# ─────────────────────────────────────────────────────────────────────────────
...
if FIGURE == "fig1_recup_subcritical":
    data_rows = sc
    labels_set = LABEL_SC
elif FIGURE == "fig2_recup_transcritical":
    data_rows = tc
    labels_set = LABEL_TC
elif FIGURE == "fig3_recup_combined":
    data_rows = pd.concat([sc, tc])
    labels_set = LABEL_SC | LABEL_TC
else:
    raise ValueError(
        f"Unknown FIGURE: {FIGURE!r}. Choose fig1_recup_subcritical, fig2_recup_transcritical, or fig3_recup_combined."
    )

# ── build figure using the EXACT same functions as the plot script ────────────
# Figure size matches the relevant figure function
_figsize_map = {
    "fig1_recup_subcritical": (12, 6.5),
    "fig2_recup_transcritical": (14, 7),
    "fig3_recup_combined": (18, 10),
}
