...
# ══════════════════════════════════════════════════════════════════════
def run_optimize(config_file, batch=False):
...
if __name__ == "__main__":
    ...
    if MODE == "optimize":
        ...
        for i, yaml_file in enumerate(BATCH_YAML_FILES, 1):
            ...
            print("-" * 70)
            try:
                run_optimize(yaml_file, batch=True)
        ...
        if not MULTISTART_BATCH_YAML_FILES:
            ...
            sys.exit(1)
        missing = [f for f in MULTISTART_BATCH_YAML_FILES if not Path(f).exists()]
        if missing:
            for f in missing:
                print(f"Error: config file not found: {f}")
            sys.exit(1)
        ...
        print("=" * 70)
        print(f"  MULTISTART BATCH — {len(MULTISTART_BATCH_YAML_FILES)} YAML files")
        print(f"  Samples per file : {MULTISTART_N_SAMPLES}")
        print(f"  Output root      : {MULTISTART_OUTPUT_DIR}")
        print("=" * 70)

        batch_summary = []
        for i, yaml_file in enumerate(MULTISTART_BATCH_YAML_FILES, 1):
            yaml_file = Path(yaml_file)
            fluid_dir = MULTISTART_OUTPUT_DIR / yaml_file.stem
            print(f"\n[{i}/{len(MULTISTART_BATCH_YAML_FILES)}] {yaml_file.name}")
            print("-" * 70)
            try:
                run_multistart(
                ...
                batch_summary.append((yaml_file.name, "✓ completed"))
            except Exception as e:
                print(f"  ERROR: {e}")
                batch_summary.append((yaml_file.name, f"✗ failed: {e}"))

        # ── Print summary ──────────────────────────────────────────────────
        print("\n" + "=" * 70)
        print("  MULTISTART BATCH — SUMMARY")
        print("=" * 70)
        for name, status in batch_summary:
            print(f"  {name:<45} {status}")
        print("=" * 70 + "\n")

        # ── Save summary to txt file ───────────────────────────────────────
        MULTISTART_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        summary_timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        summary_path = (
            MULTISTART_OUTPUT_DIR / f"multistart_batch_summary_{summary_timestamp}.txt"
        )
        n_ok = sum(1 for _, s in batch_summary if s.startswith("✓"))
        n_bad = len(batch_summary) - n_ok
        with open(summary_path, "w", encoding="utf-8") as _f:
            _f.write("=" * 70 + "\n")
            _f.write("  MULTISTART BATCH — SUMMARY\n")
            _f.write(f"  Completed    : {summary_timestamp}\n")
            _f.write(f"  YAML files   : {len(batch_summary)}\n")
            _f.write(f"  Successful   : {n_ok}\n")
            _f.write(f"  Failed       : {n_bad}\n")
            _f.write(f"  Samples/file : {MULTISTART_N_SAMPLES}\n")
            _f.write(f"  Output dir   : {MULTISTART_OUTPUT_DIR}\n")
            _f.write("=" * 70 + "\n\n")
            for name, status in batch_summary:
        ...
        print(
            f"Error: MODE must be 'optimize', 'sweep', 'pcond_sweep', 'k_sensitivity', 'multistart', 'multistart_batch', or 'parametric_study', got '{MODE}'"
        )
        sys.exit(1)
