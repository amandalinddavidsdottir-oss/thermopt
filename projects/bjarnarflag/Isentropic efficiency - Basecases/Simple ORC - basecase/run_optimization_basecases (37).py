...
#
#  "multistart_batch" — Run multistart LHS optimisation on multiple YAML files
#                  in sequence. Each YAML gets the same number of LHS samples.
#                  Results for each fluid are saved in a separate subfolder.
#                  A combined summary table is printed at the end.
#                  → Set MULTISTART_BATCH_YAML_FILES, MULTISTART_N_SAMPLES,
#                     MULTISTART_OUTPUT_DIR.
#
MODE = "multistart_batch"  # "optimize" | "sweep" | "pcond_sweep" | "k_sensitivity" | "multistart" | "multistart_batch" | "parametric_study" | "batch"
...
MULTISTART_N_SAMPLES = 2  # number of LHS starting points (50–100 recommended)
MULTISTART_OUTPUT_DIR = Path(__file__).parent / "results" / "multistart"

# ──────────────────────────────────────────── multistart_batch settings ──────────────────────────────────────────────────────────────────────────────────────────────
# Used by: multistart_batch
# List of YAML files to run multistart on — one after another.
# Each fluid gets its own subfolder inside MULTISTART_OUTPUT_DIR named after
# the YAML file stem (e.g. results/multistart/Toluene_simple_basecase/).
# MULTISTART_N_SAMPLES and MULTISTART_OUTPUT_DIR are shared with multistart mode.
# Each entry is either a Path (use fluid from YAML) or a tuple (Path, fluid_name)
# to override the working fluid for transcritical runs.
MULTISTART_BATCH_YAML_FILES = [
    Path(__file__).with_name("Toluene_simple_basecase.yaml"),
    Path(__file__).with_name("Toluene_recup_basecase.yaml"),
    (Path(__file__).with_name("Toluene_simple_basecase.yaml"), "cis-2-Butene"),
    (Path(__file__).with_name("Toluene_recup_basecase.yaml"), "cis-2-Butene"),
    Path(__file__).with_name("Toluene_dp_basecase.yaml"),
    Path(__file__).with_name("Toluene_dp_recup_basecase.yaml"),
    Path(__file__).with_name("Toluene_dp_twosource_basecase.yaml"),
    Path(__file__).with_name("Toluene_dp_twosource_recup_basecase.yaml"),
...
def _multistart_batch_worker(args):
    """Module-level worker for parallel multistart batch — required for multiprocessing on Windows."""
    entry, n_samples, output_dir = args
    if isinstance(entry, tuple):
        yaml_file, fluid = entry
    else:
        yaml_file, fluid = entry, None
    yaml_file = Path(yaml_file)
    label = yaml_file.stem + (f"_{fluid}" if fluid else "")
    fluid_dir = Path(output_dir) / label
    try:
        run_multistart(
...
def _run_single_lhs_worker(args):
    """
    Module-level worker that runs ONE LHS sample optimisation.

    Used by multistart_batch to flatten the (YAML × LHS-sample) loop into a
    single mp.Pool, so every individual optimisation runs in its own process.

    Returns a row dict matching the schema used by run_multistart, plus a
    special key ``_x0_dict`` carrying the converged variable values so the
    main process can reconstruct the optimised YAML without needing to pickle
    the ThermOpt cycle object.
    """
    import os
    import time
    ...
    (
        ...
        fluid_override,
        var_names,
        sample,  # 1-D array-like of floats in [0, 1], length = n_vars
        run_index,  # 0-based integer within this YAML's sample set
        run_label,  # string label used for tmp-file naming
        output_dir_str,
    ) = args
    ...
    # Use PID in the tmp filename to avoid clashes between concurrent workers
    tmp_path = (
    ...
    row = {
        ...
        "fluid_name": fluid_override,  # may be updated below
        "converged": False,
        "eta_system": None,
        "eta_cycle": None,
        "eta_exergy": None,
        ...
        "infeasibility": None,
        "exit_message": None,
        "_x0_dict": None,  # internal: used by main process to save optimised YAML
    }
    ...
    try:
        ...
        # Fluid name may come from the YAML itself
        ...
        row["heat_utilization"] = (
            round(ea.get("heat_utilization", 0) or 0, 6)
            if "heat_utilization" in ea
            else round(
                (
                    ea.get("heater_heat_flow", ea.get("total_heat_input", 0))
                    / ea.get("heater_heat_flow_max", 1)
                ),
                6,
            )
        )
        ...
        # ── Converged variable values (returned for YAML reconstruction) ─
        try:
            x0_dict = cycle.problem.x0_dict
            row["_x0_dict"] = {k: float(v) for k, v in x0_dict.items()}
...
def _save_optimized_yaml(base_config_file, cycle, out_path, ryaml):
    ...
    with open(base_config_file) as f:
        text = f.read()
    patched, _matched, _skipped = _patch_warmstart(text, x0_dict)
...
if __name__ == "__main__":
    ...
    if MODE == "optimize":
    ...
    elif MODE == "multistart_batch":
        if not MULTISTART_BATCH_YAML_FILES:
            print(
                "Error: MULTISTART_BATCH_YAML_FILES is empty — add YAML paths to the list."
            )
            sys.exit(1)
        missing = [
            f
            for f in MULTISTART_BATCH_YAML_FILES
        ...
        # ── Build one flat task list: every (YAML × LHS-sample) pair ─────────
        # This lets a single mp.Pool run ALL optimisations in parallel instead
        # of running the LHS samples sequentially inside per-YAML workers.
        #
        # Layout of task_meta: list of dicts with keys
        #   yaml_file, fluid, label, output_dir, run_index
        # used after the pool returns to aggregate results per YAML.
        # ─────────────────────────────────────────────────────────────────────
        all_tasks = []  # args tuples passed to _run_single_lhs_worker
        task_meta = []  # parallel list of dicts for result grouping
        yaml_order = []  # preserve insertion order for the summary

        _ryaml = _YAML()
        _ryaml.preserve_quotes = True

        for entry in MULTISTART_BATCH_YAML_FILES:
            if isinstance(entry, tuple):
                yaml_file, fluid = Path(entry[0]), entry[1]
            else:
            ...
            if label not in yaml_order:
                yaml_order.append(label)

            # Read design-variable names from the YAML (needed by worker)
            with open(yaml_file) as _f:
                _cfg = _ryaml.load(_f)
            if fluid is not None:
                _cfg["problem_formulation"]["fixed_parameters"]["working_fluid"][
                    "name"
                ] = fluid
            var_names = list(_cfg["problem_formulation"]["design_variables"].keys())

            # Generate the same LHS sample set as run_multistart (seed=42)
            sampler = LatinHypercube(d=len(var_names), seed=42)
            samples = sampler.random(n=MULTISTART_N_SAMPLES)  # (n_samples, n_vars)
            ...
            for i, sample in enumerate(samples):
                all_tasks.append(
                    (
                        ...
                        fluid,
                        var_names,
                        sample,  # array-like of floats in [0,1]
                        i,  # run_index (0-based)
                        label,  # run_label for tmp-file naming
                        str(fluid_dir),
                    )
                )
                task_meta.append(
        ...
        n_workers = min(n_total, mp.cpu_count())

        print("=" * 70)
        print(f"  MULTISTART BATCH (fully parallel flat pool)")
        print(f"  YAML files       : {len(MULTISTART_BATCH_YAML_FILES)}")
        print(f"  Samples per file : {MULTISTART_N_SAMPLES}")
        print(
            f"  Total tasks      : {n_total}  ({len(MULTISTART_BATCH_YAML_FILES)} × {MULTISTART_N_SAMPLES})"
        )
        print(f"  Workers          : {n_workers} (of {mp.cpu_count()} CPUs available)")
        print(f"  Output root      : {MULTISTART_OUTPUT_DIR}")
        print("=" * 70)

        # ── Run every optimisation in parallel ────────────────────────────────
        with mp.Pool(processes=n_workers) as pool:
            all_rows = pool.map(_run_single_lhs_worker, all_tasks)

        # ── Aggregate results per YAML ────────────────────────────────────────
        from collections import defaultdict

        rows_by_label = defaultdict(list)
        meta_by_label = {}
        for row, meta in zip(all_rows, task_meta):
            lbl = meta["label"]
            rows_by_label[lbl].append(row)
            meta_by_label[lbl] = meta  # same yaml_file / output_dir for all

        batch_summary = []  # (label, ok, err_msg) for final table

        for label in yaml_order:
            ...
            try:
                df = pd.DataFrame(rows).drop(columns=["_x0_dict"], errors="ignore")
                timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                csv_path = out_dir / f"multistart_results_{run_label}_{timestamp}.csv"
                xlsx_path = out_dir / f"multistart_results_{run_label}_{timestamp}.xlsx"
                df.to_csv(csv_path, index=False)
                df.to_excel(xlsx_path, index=False, sheet_name="Multistart")

                # Find the best converged run
                best_eta = -1.0
                best_row = None
                for row in rows:
                    if row.get("converged") and (row.get("eta_system") or 0) > best_eta:
                        best_eta = row["eta_system"]
                        best_row = row

                # Save optimised YAML from the best run's x0_dict
                optimized_yaml_path = None
                if best_row is not None and best_row.get("_x0_dict"):
                    optimized_yaml_path = (
                        out_dir / f"optimized_{run_label}_{timestamp}.yaml"
                    )
                    with open(yaml_file) as _f:
                        _text = _f.read()
                    _patched, _, _ = _patch_warmstart(_text, best_row["_x0_dict"])
                    with open(optimized_yaml_path, "w") as _f:
                        _f.write(_patched)

                n_ok = int(df["converged"].sum()) if "converged" in df.columns else 0
                n_bad = len(rows) - n_ok

                print()
                print("=" * 70)
                print(f"  {label}")
                print(f"  Converged : {n_ok}/{len(rows)}")
                if best_row is not None:
                    print(
                        f"  Best η_sys: {best_eta * 100:.4f}%  "
                        f"W_net = {best_row.get('W_net_kW', '?')} kW"
                    )
                print(f"  CSV       : {csv_path}")
                print(f"  Excel     : {xlsx_path}")
                if optimized_yaml_path:
                    print(f"  Best YAML : {optimized_yaml_path}")
                print("=" * 70)

                batch_summary.append((label, True, None))

            except Exception as _e:
                batch_summary.append((label, False, traceback.format_exc()))
                print(f"\n  ERROR aggregating {label}: {_e}")

        # ── Final summary table ───────────────────────────────────────────────
        print()
        print("=" * 70)
        print("  MULTISTART BATCH — SUMMARY")
        print("=" * 70)
        n_ok = sum(1 for _, ok, _ in batch_summary if ok)
        n_fail = len(batch_summary) - n_ok
        for lbl, ok, err in batch_summary:
            status = "✓ completed" if ok else "✗ FAILED"
            print(f"  {lbl:<55s}  {status}")
            if not ok and err:
                for line in err.strip().split("\n")[-3:]:
                    print(f"    {line}")
        print()
        print(f"  Completed : {n_ok}/{len(batch_summary)}")
        if n_fail:
