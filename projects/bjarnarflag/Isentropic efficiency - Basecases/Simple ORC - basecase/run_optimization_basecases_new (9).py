...
#                     MULTISTART_OUTPUT_DIR.
#
MODE = "optimize"  # "optimize" | "sweep" | "pcond_sweep" | "k_sensitivity" | "multistart" | "multistart_batch" | "parametric_study" | "batch"
...
CONFIG_FILE = Path(__file__).with_name("Toluene_dp_twosource_recup_basecase.yaml")
...
MULTISTART_N_SAMPLES = 100  # number of LHS starting points (50–100 recommended)
...
if __name__ == "__main__":
    ...
    if MODE == "optimize":
        ...
        n_total = len(tasks)
        n_workers = min(
            n_total, mp.cpu_count(), 4
        )  # capped at 4 to stay within memory budget
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
