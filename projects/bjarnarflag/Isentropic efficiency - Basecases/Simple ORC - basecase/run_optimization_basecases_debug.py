...
MULTISTART_N_SAMPLES = 1  # DEBUG: set to 1 to inspect a single LHS tmp file
...
MULTISTART_BATCH_YAML_FILES = [
    Path(__file__).with_name("Toluene_simple_basecase.yaml"),  # DEBUG: only one entry
    # Path(__file__).with_name("Toluene_recup_basecase.yaml"),
    ...
    try:
        ...
        for j, var_name in enumerate(var_names):
            ...
            if isinstance(vmin, (int, float)) and isinstance(vmax, (int, float)):
                ...
                # Expression bounds — write as a weighted combination that
                # thermopt can evaluate symbolically.
                t1 = round(1.0 - t, 8)
                t2 = round(t, 8)
        ...
        # ── DEBUG: pause so you can inspect the tmp YAML before it runs ──
        print(f"\n[DEBUG] LHS tmp file written to:\n  {tmp_path}")
        print("[DEBUG] Open that file now and check the value: fields look sensible.")
        input(
            "[DEBUG] Press Enter to continue with the optimiser (or Ctrl+C to abort)...\n"
        )
        ...
        row["heat_utilization"] = (
            ...
            if "heat_utilization" in ea
                ...
                / max(ea.get("heater_heat_flow_max", 1), 1e-12),
                6,
            )
        )
        try:
        ...
        if tmp_path.exists():
            try:
                pass  # DEBUG: unlink disabled — tmp file kept for inspection
                # tmp_path.unlink()
            except OSError:
                pass
...
if __name__ == "__main__":
    ...
    if MODE == "optimize":
        ...
        # ── Parse every entry in the main process ─────────────────────
        # We generate all LHS samples here (seed=42 per entry, reproducible)
        # and extract var_names + bounds from each YAML so the worker receives
        # only plain picklable data — no Path objects, no ruamel trees.
        ryaml_main = YAML()
        ryaml_main.preserve_quotes = True

        entry_meta = (
            []
        )  # (yaml_file, fluid, label, fluid_dir, var_names, bounds, samples)
        for entry in MULTISTART_BATCH_YAML_FILES:
            ...
            var_names = list(dvars.keys())
            bounds = [(dvars[v]["min"], dvars[v]["max"]) for v in var_names]

            # Same seed as run_multistart (42) → identical LHS to a single run.
            sampler = LatinHypercube(d=len(var_names), seed=42)
            samples = sampler.random(
                n=MULTISTART_N_SAMPLES
        ...
        # Every (entry × sample) pair becomes one independent task.
        # This gives the pool maximum scheduling freedom — it never idles
        # waiting for one slow entry to finish while other CPUs are free.
        tasks = []
        for (
            yaml_file,
            fluid,
            ...
            for run_index, sample in enumerate(samples):
                tasks.append(
                    (
                        str(yaml_file),  # config_file_str
                        fluid,  # fluid_override
                        var_names,  # list[str]
                        sample,  # list[float]
                        bounds,  # list[(min, max)]
                        run_index,  # int
                        label,  # run_label
                        str(fluid_dir),  # output_dir_str
                    )
                )
        ...
        print("=" * 70)
        print(f"  MULTISTART BATCH (parallel) — {n_entries} entries")
        print(f"  Samples per entry : {MULTISTART_N_SAMPLES}")
        print(
            f"  Total tasks       : {n_total}  ({n_entries} × {MULTISTART_N_SAMPLES})"
        )
        ...
        # ── Dispatch all tasks to the pool ────────────────────────────
        # imap_unordered lets us print progress as results arrive without
        # waiting for the slowest batch.
        collected = {}  # label -> list[row]
        for lbl in [m[2] for m in entry_meta]:
            collected[lbl] = []
        ...
        with mp.Pool(processes=n_workers) as pool:
            for row in pool.imap_unordered(_multistart_sample_worker, tasks):
                ...
                print(
                    ...
                    f"run {row['run']:>3}/{MULTISTART_N_SAMPLES}   {status}"
                )

        # ── Save outputs per entry (main process) ─────────────────────
        # No nested pools here — we are back in the main process.
        print()
        print("=" * 70)
        print("  Saving per-entry outputs …")
        print("=" * 70)

        summary_rows = []  # (label, n_ok, n_total, best_eta, optimized_path)
        for (
            yaml_file,
            fluid,
            label,
            ...
            n_bad = MULTISTART_N_SAMPLES - n_ok
            print(f"\n  ── {label}")
            print(f"     Converged : {n_ok}/{MULTISTART_N_SAMPLES}   Failed: {n_bad}")
            ...
            if lhs_yaml_path:
                print(
                    f"     LHS start : {lhs_yaml_path.name}  (re-run in 'optimize' mode to reproduce best)"
                )
            ...
            summary_rows.append(
                (label, n_ok, MULTISTART_N_SAMPLES, best_eta, optimized_yaml_path)
            )

        # ── Overall summary table ──────────────────────────────────────
        print()
        print("=" * 70)
        print("  MULTISTART BATCH — FINAL SUMMARY")
        print("=" * 70)
        print(f"  {'Entry':<50}  {'Conv':>5}  {'Best η_sys':>11}  {'Optimized YAML'}")
        print(f"  {'─'*50}  {'─'*5}  {'─'*11}  {'─'*30}")
