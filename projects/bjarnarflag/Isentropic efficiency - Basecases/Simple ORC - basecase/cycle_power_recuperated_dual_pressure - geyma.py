...
def evaluate_cycle(
    ...
    # well_mass_flow_rate can be either a fixed parameter or a design variable
    mass_flow_mode = (
        "well_mass_flow_rate" in parameters or "well_mass_flow_rate" in variables
    )
    ...
    if "inlet_enthalpy" in parameters["heat_source"]:
        # Two-phase or enthalpy-specified heat source (e.g. geothermal brine)
        ...
        h_in_hot_brine = heating_fluid.get_state(
            cpx.PT_INPUTS, p_in_hot_brine, T_in_hot_brine
        ).h
    T_HS1 = heat_source_mid_temp
    p_out_hot_hp = p_in_hot_brine * (1.0 - dp_hp_evap_h)
    h_out_hot_hp = heating_fluid.get_state(cpx.PT_INPUTS, p_out_hot_hp, T_HS1).h
