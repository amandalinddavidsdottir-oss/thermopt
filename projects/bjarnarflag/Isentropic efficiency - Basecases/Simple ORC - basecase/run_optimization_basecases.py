...
def run_pcond_sweep(
    template_yaml, fluids, output_dir, turbine_model="isentropic", n_stages=4, RPM=1000
):
    """
    For each candidate fluid, run a full ThermOpt optimization using the
    template YAML. Only the working fluid name is substituted (and
    n_stages/RPM when turbine_model="astolfi").

    turbine_model : "isentropic"  — fixed efficiency from YAML, no patching.
                    "astolfi"     — patches n_stages/RPM and uses the
                                   Macchi-Astolfi correlation; Ns-invalid
                                   results are flagged on the plot.
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for i, (fluid_name, category) in enumerate(fluids.items(), 1):
        ...
        try:
            ...
            # Astolfi mode: also patch n_stages and RPM
            if turbine_model == "astolfi":
    ...
    plot_path = _plot_pcond_efficiency(valid, flagged, output_dir, tag)
...
def _plot_pcond_efficiency(valid, flagged, output_dir, tag):
    """
    ...
    # ── Fluid name labels ─────────────────────────────────────────────
    for r in valid + flagged:  # flagged is [] in isentropic mode
