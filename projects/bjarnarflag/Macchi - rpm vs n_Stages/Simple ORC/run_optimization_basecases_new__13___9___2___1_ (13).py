...
CONFIG_FILE = Path(__file__).with_name(
    "Toluene_recup_basecase_optimized_Macchi.yaml"
)
...
PARAMETRIC_RPM_LIST = [1000, 1500, 3000]  # 1000 first (Roberto's recommendation), transition_vars saved from RPM=1500

# Mapping from RPM to number of parallel turbines.
# Roberto's recommendation: RPM=1000 → 1 turbine, RPM=1500 → 2 turbines, RPM=3000 → 4 turbines.
# Set to None to keep whatever n_turbines_parallel is in the base YAML.
PARAMETRIC_RPM_TO_N_TURBINES = {
    1000: 1,
    1500: 2,
    3000: 4,
}
...
def run_parametric_nstages_rpm(
    ...
    for n_stages in n_stages_list:
        ...
        for rpm in rpm_list:
            ...
            # For RPM=3000: reset warm_vars to RPM=1500 result so it starts
            # from the correct high-evaporation region rather than RPM=1000
            if rpm == 3000 and rpm1500_vars:
                ...
                print("(warmstart reset to RPM=1500 values)", end=" ", flush=True)
            cycle.set_config_value(rpm_config_path, rpm)

            # Set n_turbines_parallel based on RPM if mapping is provided
            if rpm_to_n_turbines is not None and rpm in rpm_to_n_turbines:
                n_turb = rpm_to_n_turbines[rpm]
            ...
            try:
                ...
                # Save transition_vars and rpm1500_vars from RPM=1500.
                # RPM=1500 consistently converges to the correct high-evaporation
                # operating point and gives the best warmstart for the next
                # n_stages block and for RPM=3000.
                # Note: saved from rpm==1500 explicitly, not rpm_list[0],
                # so this works regardless of RPM order.
                if converged and rpm == 1500:
