...
CONFIG_FILE = Path(__file__).with_name(
    "Isopentane_recup_basecase_optimized_Macchi.yaml"
)
...
# Base YAML files — one per ORC configuration.
# Each entry is one n=1 YAML (no intermediate pressures).
# - ONE entry   -> runs sequentially in the main process (easy to debug).
# - MANY entries -> dispatches each config to a parallel worker so all
#                  configurations run their full n_stages x RPM sweep
#                  simultaneously.  The warm start chain inside each
#                  worker is fully preserved — only independent configs
#                  run in parallel, never runs within the same sweep.

PARAMETRIC_BASE_YAMLS = [
...
#PARAMETRIC_BASE_YAMLS = [
    #Path(__file__).with_name("Toluene_recup_basecase_optimized_Macchi (3).yaml"),
    #Path(__file__).with_name("Toluene_simple_basecase_optimized_Macchi.yaml"),
    # Path(__file__).with_name("Toluene_recup_basecase_optimized_Macchi_singleturb.yaml"),
    # Path(__file__).with_name("Toluene_simple_basecase_optimized_Macchi_single_turb.yaml"),
    # Path(__file__).with_name("Toluene_dp_twosource_basecase_optimized_Macchi.yaml"),
    # Path(__file__).with_name("Toluene_dp_twosource_recup_basecase_optimized_Macchi.yaml"),
    # Path(__file__).with_name("Toluene_simple_transcritical_basecase_optimized_Macchi.yaml"),
    # Path(__file__).with_name("Toluene_recup_transcritical_basecase_optimized_Macchi.yaml"),
#]

# Number of parallel worker processes.
# Each worker runs one config's full sequential n_stages x RPM sweep.
# Recommended: set to min(len(PARAMETRIC_BASE_YAMLS), available_cores).
...
# for all subsequent n_stages blocks. Starting from n=2 is physically justified.

# RPM values for the inner loop.
# Each entry is (RPM, n_turbines_parallel).
# Warmstart strategy:
#   (1000, 1)  — starts from transition_vars (RPM=1500,2 of previous block)
#   (1500, 2)  — carries from (1000,1); saves transition_vars and rpm1500_vars on convergence
#   (1500, 4)  — resets to rpm1500_vars before running
#   (3000, 4)  — resets to rpm1500_vars before running
#   (3000, 6)  — carries from (3000,4) result
PARAMETRIC_RPM_TURBINES_LIST = [
    (1000, 1),
    (1500, 2),
    (1500, 4),
    (3000, 4),
...
# Derived lists for backward compatibility with total run count display
PARAMETRIC_RPM_LIST = [rpm for rpm, _ in PARAMETRIC_RPM_TURBINES_LIST]
PARAMETRIC_RPM_TO_N_TURBINES = {rpm: n for rpm, n in PARAMETRIC_RPM_TURBINES_LIST}

# Where to save the combined CSV + Excel results.
# Each config gets its own subfolder named after the YAML stem.
...
PARAMETRIC_INTERP_P_MIN_EXPR = "$working_fluid.liquid_at_ambient_temperature.p"
PARAMETRIC_INTERP_P_MAX_EXPR = "2*$working_fluid.critical_point.p"
...
PCOND_CYCLE = "recuperated"  # "simple" | "recuperated"

# Single YAML template per cycle topology — used for ALL fluids in pcond_sweep.
PCOND_TEMPLATES = {
    "simple": Path(__file__).with_name("Toluene_simple_basecase_optimized.yaml"),
    "recuperated": Path(__file__).with_name("Toluene_recup_basecase_optimized.yaml"),
}
...
# The fluid candidates follow the selection of Astolfi (2014), comprising all
# pure ORC-relevant fluids available in CoolProp, excluding chlorinated
# compounds due to their high ozone depletion potential.
#
# This list corresponds to Tables 4.3 and 4.4 of:
#   Astolfi, M. (2014). An Innovative Approach for the Techno-Economic
#   Optimization of Organic Rankine Cycles. PhD Thesis, Politecnico di Milano.
#
# Four fluids present in Astolfi's original list are omitted due to CoolProp
# database limitations: methyl-cyclohexane, n-propyl-cyclohexane,
# perfluorobutane (C4F10), and perfluorocyclopropane (C3F6).
#
...
#     (Path(__file__).with_name("Toluene_simple_basecase.yaml"),             None, 250),
#     (Path(__file__).with_name("Toluene_recup_basecase.yaml"),              None, 250),
#     (Path(__file__).with_name("Toluene_dp_basecase.yaml"),                 None, 250),
#     (Path(__file__).with_name("Toluene_dp_recup_basecase.yaml"),           None, 250),
#     (Path(__file__).with_name("Toluene_dp_twosource_basecase.yaml"),       None, 250),
#     (Path(__file__).with_name("Toluene_dp_twosource_recup_basecase.yaml"), None, 250),
# ]

MULTISTART_YAML_FILES = [
    (Path(__file__).with_name("Recup_basecase - transcritical.yaml"), None, 250),
...
def _extract_tq_rows(cycle):
    """Extract HX heat duty, temperatures and pinch from cycle components.
    Uses bracketed component names e.g. [heater] so keywords never
    accidentally match a different component (e.g. heater vs preheater).
    """
    rows = []
    ...
    for name in hx_order:
        hx = components.get(name)
        if hx is None:
            continue
        ea = hx.get("energy_analysis", {})
        q = abs(ea.get("Q_hot", ea.get("Q_cold", 0))) / 1e3
        if q < 0.1:  # skip truly inactive components
            continue
        tag = f"[{name}]"  # bracketed tag — unique, no substring collisions
        rows.append((f"{tag} heat duty [kW]", round(q, 3)))
        try:
            rows.append(
                (
                    f"{tag} hot side inlet [°C]",
                    round(hx["hot_side"]["state_in"].T - 273.15, 2),
                )
            )
            rows.append(
                (
                    f"{tag} hot side outlet [°C]",
                    round(hx["hot_side"]["state_out"].T - 273.15, 2),
                )
            )
        except Exception:
            pass
        try:
            rows.append(
                (
                    f"{tag} cold side inlet [°C]",
                    round(hx["cold_side"]["state_in"].T - 273.15, 2),
                )
            )
            rows.append(
                (
                    f"{tag} cold side outlet [°C]",
                    round(hx["cold_side"]["state_out"].T - 273.15, 2),
                )
            )
        except Exception:
            pass
        try:
            dt = hx.get("temperature_difference")
            pinch = float(dt.min() if hasattr(dt, "min") else min(dt))
            rows.append((f"{tag} pinch point [K]", round(pinch, 4)))
        except Exception:
            pass
    return rows


def _extract_mass_flow_rows(cycle):
    """
    Extract all mass flow rates from the cycle for the comparison sheet.

    Working fluid:
      - Total cycle mass flow (all configs)
      - HP circuit mass flow  (dual-pressure configs only)
      - LP circuit mass flow  (dual-pressure configs only)
      HP/LP split is computed from mass_split_fraction in x0_dict:
        m_HP = mass_split_fraction * m_total
        m_LP = (1 - mass_split_fraction) * m_total

    Heat source:
      - Single brine total  (simple / recuperated / DP single-brine)
      - HP brine (well 2)   (DP dual-brine configs)
      - LP brine (wells 1/3/4) (DP dual-brine configs)
      - Dual brine total

    Heat sink:
      - Cooling water total
    """
    ea = cycle.problem.cycle_data.get("energy_analysis", {})
    rows = [("══ MASS FLOW RATES ══",)]

    # ── Working fluid ────────────────────────────────────────────────
    m_wf_total = ea.get("mass_flow_working_fluid")
    if m_wf_total is not None:
        rows.append(("Mass flow WF total [kg/s]", round(m_wf_total, 3)))

    # HP / LP circuit split — only present in dual-pressure topologies
    try:
        x0 = cycle.problem.x0_dict
        split = float(x0.get("mass_split_fraction", 0))
        # Only write if this is genuinely a dual-pressure config
        # (mass_split_fraction exists AND hp_evaporator component present)
        comps = cycle.problem.cycle_data.get("components", {})
        if split > 0 and "hp_evaporator" in comps and m_wf_total is not None:
            m_hp = split * m_wf_total
            m_lp = (1.0 - split) * m_wf_total
            rows.append(("Mass flow WF HP circuit [kg/s]", round(m_hp, 3)))
            rows.append(("Mass flow WF LP circuit [kg/s]", round(m_lp, 3)))
            rows.append(("WF mass split fraction HP [-]", round(split, 4)))
    except Exception:
        pass

    # ── Heat source (brine) ──────────────────────────────────────────
    if "mass_flow_heating_fluid" in ea:
        # Single brine (all four wells mixed, or wells 1/3/4)
        rows.append(
            (
                "Mass flow brine single source [kg/s]",
                round(ea["mass_flow_heating_fluid"], 3),
            )
        )
    if "mass_flow_brine_hp" in ea:
        mhp = ea["mass_flow_brine_hp"]
        mlp = ea["mass_flow_brine_lp"]
        rows.append(("Mass flow brine HP source — well 2 [kg/s]", round(mhp, 3)))
        rows.append(("Mass flow brine LP source — wells 1/3/4 [kg/s]", round(mlp, 3)))
        rows.append(("Mass flow brine total [kg/s]", round(mhp + mlp, 3)))

    # ── Heat sink (cooling water) ────────────────────────────────────
    if "mass_flow_cooling_fluid" in ea:
        rows.append(
            ("Mass flow cooling water [kg/s]", round(ea["mass_flow_cooling_fluid"], 3))
        )
    return rows
...
def _append_extra_sheets(excel_path, cycle, config_file=None):
    ...
    validation_rows = list(get_validation_data(cycle, config_file=config_file))
    turbine_rows = get_turbine_data(cycle)  # None if isentropic mode
    tq_rows = _extract_tq_rows(cycle)  # replaces empty get_tq_summary_data
    # Append mass-flow rows directly into validation_checks so they are
    # always available in the flat pdict used by the comparison sheet.
    validation_rows += _extract_mass_flow_rows(cycle)
...
# ══════════════════════════════════════════════════════════════════════
def _merge_batch_to_comparison_excel(batch_dir, yaml_files, config_labels, output_path):
    """
    After a batch run, find each config's post_processing_results.xlsx and
    copy all its sheets into a single master comparison workbook.

    Sheet 1 (front): "Comparison" — key metrics for all configs side-by-side.
    Remaining sheets: "<Short label> | <original sheet name>" for every config.
    """
    import openpyxl
    ...
    # ── Style helpers ─────────────────────────────────────────────────
    _BLUE = PatternFill("solid", fgColor="1F3864")  # dark blue  — main headers
    _LBLUE = PatternFill("solid", fgColor="2E75B6")  # mid blue   — section headers
    _ALT = PatternFill("solid", fgColor="EBF0FB")  # light blue — alternate rows
    _GREEN = PatternFill("solid", fgColor="C6EFCE")  # green      — best value
    _WFONT = Font(bold=True, color="FFFFFF", size=11)
    ...
    def _hdr(ws, r, c, val, size=11):
        cell = ws.cell(row=r, column=c, value=val)
        cell.fill, cell.font, cell.border = (
            _BLUE,
            Font(bold=True, color="FFFFFF", size=size),
            _BDR,
        )
        cell.alignment = _CENTER
        return cell

    def _sec(ws, r, c_start, c_end, val):
        """Section divider spanning c_start:c_end."""
        ws.merge_cells(start_row=r, start_column=c_start, end_row=r, end_column=c_end)
        cell = ws.cell(row=r, column=c_start, value=val)
        cell.fill, cell.font, cell.border = _LBLUE, _WFONT, _BDR
        cell.alignment = _LEFT

    def _lbl(ws, r, c, val):
        cell = ws.cell(row=r, column=c, value=val)
        cell.font, cell.border = _BOLD, _BDR
        cell.alignment = _LEFT
        if r % 2 == 0:
            cell.fill = _ALT
        return cell

    def _val(ws, r, c, val, fmt="0.00"):
        cell = ws.cell(row=r, column=c, value=val)
        cell.border, cell.alignment = _BDR, _RIGHT
    ...
    def _highlight_best(ws, data_row, col_start, col_end, higher_is_better=True):
        vals = []
        ...
        if not vals:
            return
        best_c = (max if higher_is_better else min)(vals, key=lambda x: x[0])[1]
        best_cell = ws.cell(row=data_row, column=best_c)
        best_cell.fill = _GREEN
        best_cell.font = Font(bold=True, color="1F3864")

    # ── Load all source workbooks up front ────────────────────────────
    sources = {}  # label → (wb_src, param_dict)
    ordered_labels = []
    for yaml_file, label in zip(yaml_files, config_labels):
        src_path = (
            batch_dir
            / yaml_file.stem
            / "post_processing"
            / "post_processing_results.xlsx"
        )
        if not src_path.exists():
            print(f"  [SKIP] No Excel found for '{label}': {src_path}")
            continue
        try:
            wb_src = load_workbook(src_path, data_only=False)
        except Exception as e:
            print(f"  [SKIP] Could not open Excel for '{label}': {e}")
            continue
        # Build a flat key→value dict from every "Parameter | Value" sheet
        pdict = {}
        for sname in wb_src.sheetnames:
            ws = wb_src[sname]
            mr = ws.max_row or 1
            mc = ws.max_column or 2
            for row in ws.iter_rows(
                min_row=2, max_row=mr, min_col=1, max_col=mc, values_only=True
            ):
                if row and row[0] is not None:
                    key = f"{sname}::{str(row[0]).strip()}"
                    pdict[key] = row[1] if len(row) > 1 else None
        sources[label] = (wb_src, pdict)
        ordered_labels.append(label)
        print(f"  ✓ Loaded {len(wb_src.sheetnames)} sheets  ←  {label}")

    if not sources:
        print("  [ERROR] No source workbooks loaded — nothing to merge.")
        return

    # ── Helper: fuzzy-find a value across all sheets for one config ───
    def _get(label, *keywords):
        """Return first value whose key contains ALL keywords (case-insensitive)."""
        if label not in sources:
            return None
        pdict = sources[label][1]
        kws = [k.lower() for k in keywords]
        for key, val in pdict.items():
            kl = key.lower()
            if all(kw in kl for kw in kws):
                try:
                    # strip % signs and convert to float where possible
    ...
    # ── Build master workbook — comparison sheet FIRST ────────────────
    master = openpyxl.Workbook()
    master.remove(master.active)

    ws_cmp = master.create_sheet("⭐ Comparison")
    n = len(ordered_labels)
    C0 = 1  # metric label column
    CU = 2  # unit column
    CD = 3  # data columns start
    CD_END = CD + n - 1

    # Title row
    ws_cmp.merge_cells(start_row=1, start_column=1, end_row=1, end_column=CD_END)
    title = ws_cmp.cell(
        row=1, column=1, value="Bjarnarflag ORC — Configuration Comparison"
    )
    ...
    # Column headers
    _hdr(ws_cmp, 2, C0, "Metric")
    _hdr(ws_cmp, 2, CU, "Unit")
    for i, lbl in enumerate(ordered_labels):
        _hdr(ws_cmp, 2, CD + i, lbl)
    ws_cmp.freeze_panes = "C3"

    # ── Metric definitions ─────────────────────────────────────────────
    # Each tuple: (display_label, unit, fmt, higher_is_better, *keywords)
    # Keywords are searched as substrings in the flat pdict keys (sheet::param).
    # Use bracket-prefixed keys e.g. "[heater]" to avoid substring collisions.
    SECTIONS = [
        (
            "EFFICIENCIES",
            [
                (
                    "η_system  = W_net / Q_available",
                    "%",
                    "0.000",
                    True,
                    "system efficiency",
                ),
                ("η_cycle   = W_net / Q_in", "%", "0.000", True, "cycle efficiency"),
                ("η_exergy  = W_net / E_fuel", "%", "0.000", True, "exergy efficiency"),
            ],
        ),
        (
            "POWER",
            [
                ("W_net  (net system power)", "kW", "0.0", None, "net system power"),
                ("W_expander gross", "kW", "0.0", True, "gross expander power"),
                ("W_pump  (WF pump / compressor)", "kW", "0.0", False, "wf pump"),
                (
                    "W_aux   (brine + CW pumps)",
                    "kW",
                    "0.0",
                    False,
        ...
        (
            "HEAT FLOWS",
            [
                ("Q_in   (total heat input)", "kW", "0.0", False, "heat input q_in"),
                ("Q_available", "kW", "0.0", None, "available heat q_available"),
                ("Q_out  (heat rejected to CW)", "kW", "0.0", False, "q_out [kw]"),
                (
                    "Heat utilisation  Q_in/Q_avail",
                    "%",
                    "0.00",
                    True,
                    "heat utilization",
        ...
        (
            "MASS FLOW RATES — WORKING FLUID",
            [
                (
                    "ṁ WF total  (all configs)",
                    "kg/s",
                    "0.000",
                    None,
                    "mass flow wf total",
                ...
                (
                    "ṁ WF HP circuit  (DP configs only)",
                    "kg/s",
                    "0.000",
                    None,
                    "mass flow wf hp circuit",
                ),
                (
                    "ṁ WF LP circuit  (DP configs only)",
                    "kg/s",
                    "0.000",
                    None,
                    "mass flow wf lp circuit",
                ),
                (
                    "WF mass split fraction HP",
                    "—",
                    "0.0000",
                    None,
                    "wf mass split fraction hp",
        ...
        (
            "MASS FLOW RATES — HEAT SOURCE",
            [
                (
                    "ṁ brine  (single source)",
                    "kg/s",
                    "0.000",
                    None,
                    "mass flow brine single source",
                ),
                (
                    "ṁ brine HP source  (well 2)",
                    "kg/s",
                    "0.000",
                    None,
                    "mass flow brine hp source",
                ),
                (
                    "ṁ brine LP source  (wells 1/3/4)",
                    "kg/s",
                    "0.000",
                    None,
                    "mass flow brine lp source",
                ),
                (
                    "ṁ brine total  (dual brine)",
                    "kg/s",
                    "0.000",
                    None,
                    "mass flow brine total",
                ),
            ],
        ),
        (
            "MASS FLOW RATES — HEAT SINK",
            [
                ("ṁ cooling water", "kg/s", "0.000", None, "mass flow cooling water"),
            ],
        ),
        (
            "HEAT EXCHANGER PINCH POINTS",
            [
                # Brackets in tq_summary keys prevent substring collisions
                ("ΔT_min  heater  (simple ORC)", "K", "0.00", None, "[heater] pinch"),
                ("ΔT_min  HP evaporator", "K", "0.00", None, "[hp_evaporator] pinch"),
                ("ΔT_min  LP evaporator", "K", "0.00", None, "[lp_evaporator] pinch"),
                ("ΔT_min  preheater", "K", "0.00", None, "[preheater] pinch"),
                ("ΔT_min  recuperator", "K", "0.00", None, "[recuperator] pinch"),
                ("ΔT_min  condenser", "K", "0.00", None, "[cooler] pinch"),
            ],
        ),
        (
            "HX HEAT DUTIES",
            [
                ("Q  heater  (simple ORC)", "kW", "0.0", None, "[heater] heat duty"),
                ("Q  HP evaporator", "kW", "0.0", None, "[hp_evaporator] heat duty"),
                ("Q  LP evaporator", "kW", "0.0", None, "[lp_evaporator] heat duty"),
                ("Q  preheater", "kW", "0.0", None, "[preheater] heat duty"),
                ("Q  recuperator", "kW", "0.0", None, "[recuperator] heat duty"),
                ("Q  condenser", "kW", "0.0", None, "[cooler] heat duty"),
            ],
        ),
        (
            "EXERGY",
            [
                ...
                (
                    "E_D  total  (all components)",
                    "kW",
                    "0.0",
                    False,
                    "total exergy destruction",
                ),
                ("E_loss condenser", "kW", "0.0", False, "exergy loss condenser"),
                ("η_exergy  = W_net / E_fuel", "%", "0.000", True, "exergy efficiency"),
            ],
        ),
    ]
    ...
    row = 3
    for sec_title, metrics in SECTIONS:
        _sec(ws_cmp, row, C0, CD_END, f"  {sec_title}")
        row += 1
        for entry in metrics:
            disp_lbl, unit, fmt, hib, *kws = entry
            _lbl(ws_cmp, row, C0, disp_lbl)
            _val(ws_cmp, row, CU, unit, fmt=None)
            ws_cmp.cell(row=row, column=CU).alignment = _LEFT
            for i, lbl in enumerate(ordered_labels):
                v = _get(lbl, *kws)
                _val(ws_cmp, row, CD + i, v, fmt=fmt)
            if hib is not None:
                _highlight_best(ws_cmp, row, CD, CD_END, higher_is_better=hib)
            row += 1

    # Column widths for comparison sheet
    ws_cmp.column_dimensions["A"].width = 40
    ws_cmp.column_dimensions["B"].width = 8
    for i in range(n):
        ws_cmp.column_dimensions[openpyxl.utils.get_column_letter(CD + i)].width = max(
            16, len(ordered_labels[i]) + 2
        )

    # ── Now append per-config sheets ──────────────────────────────────
    for label in ordered_labels:
        wb_src = sources[label][0]
        short = label[:18]
        for sheet_name in wb_src.sheetnames:
            ws_src = wb_src[sheet_name]
            dest_name = f"{short} | {sheet_name}"[:31]
            if dest_name in master.sheetnames:
                dest_name = dest_name[:28] + "_2"
            ws_dst = master.create_sheet(dest_name)
            max_row = ws_src.max_row or 1
            max_col = ws_src.max_column or 1
            for row_cells in ws_src.iter_rows(
                min_row=1, max_row=max_row, min_col=1, max_col=max_col
            ):
                for cell in row_cells:
                    nc = ws_dst.cell(row=cell.row, column=cell.column, value=cell.value)
                    if cell.has_style:
                        nc.font, nc.fill = cell.font.copy(), cell.fill.copy()
    ...
    print(f"\n  ✓ Comparison workbook saved → {output_path}\n")
...
def _extract_results(cycle, config_file, n_stages, RPM):
    ...
    # Get n_turbines_parallel from cycle config
    try:
        n_turbines_parallel = cycle.config["problem_formulation"]["fixed_parameters"]["expander"]["n_turbines_parallel"]
    except (KeyError, TypeError):
        n_turbines_parallel = 1

    result = {
        "config": config_name,
        "n_stages": n_stages,
        "RPM": RPM,
        "n_turbines_parallel": n_turbines_parallel,
        "converged": True,
        "ns_invalid": False,
    }
...
def run_parametric_nstages_rpm(
    ...
    rpm_to_n_turbines=None,
    n_turbines_config_path="problem_formulation.fixed_parameters.expander.n_turbines_parallel",
    rpm_turbines_list=None,
):
    """
    ...
    # Use rpm_turbines_list if provided, else fall back to rpm_list
    _loop_pairs = rpm_turbines_list if rpm_turbines_list else [(rpm, rpm_to_n_turbines.get(rpm) if rpm_to_n_turbines else None) for rpm in rpm_list]
    total = len(n_stages_list) * len(_loop_pairs)
    run_n = 0
    ...
    if rpm_turbines_list:
        print(f"  RPM × turbines : {rpm_turbines_list}")
    else:
    ...
    rpm1500_vars = {}  # warm_vars saved from RPM=1500 run — used as warmstart for RPM=3000
    transition_vars = (
        {}
    )  # warm_vars saved from RPM=1500 (first RPM) of each n_stages block
    # Used for the n_stages boundary transition — RPM=1500 consistently
    # converges to the correct high-evaporation operating point
    ...
    for n_stages in n_stages_list:
        ...
        # Intermediate pressure geometric split is computed in step 4b below,
        # AFTER the cycle-level warmstart has been applied, so that p_in and
        # p_out are taken from the same warmstart values as all other variables.
        # This ensures internal consistency between intermediate pressures and
        # the rest of the cycle warmstart.
        ...
        #       Use transition_vars (from RPM=1500 of previous n_stages block)
        ...
        if n_carried:
            src_label = (
                "transition_vars (RPM=1500)" if transition_vars else "warm_vars"
            )
            print(
        ...
        # ── 4b. Compute intermediate pressure geometric split ─────────
        # Done AFTER step 4 so p_in/p_out come from dvars which now holds
        # the cycle-level warmstart just applied — ensuring intermediate
        # pressures are always internally consistent with all other warmstart values.
        p_in = dvars.get(expander_inlet_p_var, {}).get("value")
        p_out = dvars.get(expander_outlet_p_var, {}).get("value")
        if p_in is None or p_out is None or not (p_in > p_out > 0):
            raise ValueError(
                f"Cannot compute intermediate pressure warm start: "
                f"'{expander_inlet_p_var}' or '{expander_outlet_p_var}' "
                f"not found in design_variables or have invalid values. "
                f"Check that these variable names match your YAML."
            )
        print(f"    Geometric split: p_in={p_in/1e5:.4f} bar, p_out={p_out/1e5:.4f} bar")
        # Pre-compute all intermediate-pressure geometric guesses so each
        # intermediate knows its neighbors' guesses before we set bounds.
        #   p_int_i = p_in * (p_out/p_in)^(i/n_stages)     for i = 1..n-1
        guesses = [
            float(p_in * (p_out / p_in) ** (idx / n_stages))
            for idx in range(1, n_stages)
        ]
        # Margin pulled off each neighbor to keep the bound strictly inside
        # (neighbor, neighbor). 0.1% is tight enough to prevent stage
        # inversion and loose enough to avoid numerical tangency.
        _BOUND_MARGIN = 0.001
        for i, var_name in enumerate(existing):
            # Upper neighbor (the pressure immediately above this stage):
            #   first intermediate (i == 0)          -> expander inlet p_in
            #   otherwise                            -> previous intermediate's guess
            upper_neighbor = p_in if i == 0 else guesses[i - 1]
            # Lower neighbor (the pressure immediately below this stage):
            #   last intermediate (i == len - 1)     -> expander outlet p_out
            #   otherwise                            -> next intermediate's guess
            lower_neighbor = (
                p_out if i == len(existing) - 1 else guesses[i + 1]
            )
            bound_min = lower_neighbor * (1.0 + _BOUND_MARGIN)
            bound_max = upper_neighbor * (1.0 - _BOUND_MARGIN)
            p_guess = guesses[i]

            # Write the neighbor-derived box bound + geometric-split guess.
            # The bound (numeric literal) overrides any string expression
            # ($working_fluid...) that was placed in step 1. The physical
            # ordering constraint (p_in > p1 > ... > p_out) still runs via
            # the YAML constraint list; this bound just stops SLSQP's
            # finite-difference probe from stepping *outside* it (where
            # Dh_is_stg goes negative and **0.25 NaNs).
            dvars[var_name]["min"] = bound_min
            dvars[var_name]["max"] = bound_max
            dvars[var_name]["value"] = p_guess
            cycle.set_config_value(
                f"problem_formulation.design_variables.{var_name}.value",
                p_guess,
                reload=False,
            )
            print(
                f"    ↺ set {var_name} = {p_guess:.4e} Pa  "
                f"({p_guess/1e5:.4f} bar)   "
                f"bounds [{bound_min/1e5:.4f}, {bound_max/1e5:.4f}] bar"
            )
        ...
        # ══════════════════════════════════════════════════════════════
        # INNER LOOP — (RPM, n_turbines) pairs
        # ══════════════════════════════════════════════════════════════
        for rpm, n_turb in _loop_pairs:
            run_n += 1
            print(
                f"\n  [{run_n:>2}/{total}] n_stages={n_stages}, RPM={rpm:>5}, n_turb={n_turb} ... ",
                end="",
                flush=True,
            )

            # 6. Warmstart reset strategy:
            # (1500,4): reset to rpm1500_vars — n_turbines change means different mass flow
            # (3000,4): reset to rpm1500_vars — best available high-pressure solution
            # (3000,6): carry forward from (3000,4) — same RPM, similar conditions
            # (1000,1) and (1500,2): carry forward from warm_vars naturally
            if (rpm == 1500 and n_turb == 4) and rpm1500_vars:
                warm_vars = dict(rpm1500_vars)
                ...
                print("(warmstart reset to RPM=1500,2turb values)", end=" ", flush=True)
            elif (rpm == 3000 and n_turb == 4) and rpm1500_vars:
                warm_vars = dict(rpm1500_vars)
                for k, v in warm_vars.items():
                    if k in dvars:
                        cycle.set_config_value(
            ...
            # Set n_turbines_parallel
            if n_turb is not None:
                cycle.set_config_value(n_turbines_config_path, n_turb)
                print(f"(n_turbines_parallel={n_turb})", end=" ", flush=True)

            # ── Pre-flight diagnostics ──────────────────────────────
            # (1) Confirm the intermediate pressure(s) actually made it
            #     into the x0 the optimizer will use.
            # (2) Verify the expander inlet state is not in the
            #     two-phase dome (crashes SP/Ns at first eval).
            try:
                import CoolProp.CoolProp as _CP
                _x0 = dict(cycle.problem.x0_dict)
                _fluid = cycle.config["problem_formulation"][
                    "fixed_parameters"
                ]["working_fluid"]["name"]
                print()
                # (1) intermediate pressures as seen by the optimizer
                _inter = sorted(
                    (k, v) for k, v in _x0.items()
                    if k.startswith(interp_var_prefix + "_")
                )
                for _k, _v in _inter:
                    print(f"     x0[{_k}] = {_v:.6e} Pa = {_v/1e5:.4f} bar")
                # (2) expander inlet phase check
                _p_in = _x0.get("expander_inlet_pressure")
                _h_in = _x0.get("expander_inlet_enthalpy")
                if _p_in is not None and _h_in is not None:
                    _q = _CP.PropsSI("Q", "H", _h_in, "P", _p_in, _fluid)
                    _T = _CP.PropsSI("T", "H", _h_in, "P", _p_in, _fluid) - 273.15
                    if 0.0 <= _q <= 1.0:
                        print(
                            f"     ⚠  expander inlet: T={_T:.2f} C, "
                            f"quality={_q:.4f}  TWO-PHASE (bad warmstart)"
                        )
                    else:
                        print(
                            f"     expander inlet: T={_T:.2f} C, "
            ...
            # 7. Run optimizer
            #    Wrap in np.errstate so the first NaN-producing operation
            #    raises FloatingPointError *at its origin line* instead of
            #    silently propagating through the cycle evaluation and
            #    surfacing later as a confusing HmassP_INPUTS(nan, ...)
            #    traceback. Only affects this block — restored on exit.
            try:
                import numpy as _np
                with _np.errstate(invalid="raise", divide="raise"):
                    cycle.run_optimization()
                ...
                result["converged"] = converged
                result["exit_message"] = str(cycle.solver.message)

                # Save transition_vars and rpm1500_vars from RPM=1500 n_turb=2 only.
                ...
                print(f"✗  FAILED: {e}")
                traceback.print_exc()
                # ── Crash-time diagnostic ───────────────────────────
                # Read the x the optimizer was evaluating at crash time
                # (cycle.problem.x0_dict is updated each fitness call),
                # then ask CoolProp: at the expander inlet's entropy,
                # does the isentrope hit p_cond inside the two-phase
                # dome? quality in (0, 1) ⇒ "isentrope into dome"
                # hypothesis confirmed — fix by adding an outlet
                # superheat or quality constraint in the YAML.
                try:
                    import CoolProp.CoolProp as _CP
                    _xd = dict(cycle.problem.x0_dict)
                    _fluid = cycle.config["problem_formulation"][
                        "fixed_parameters"
                    ]["working_fluid"]["name"]
                    _p_in = _xd["expander_inlet_pressure"]
                    _h_in = _xd["expander_inlet_enthalpy"]
                    _p_cond = _xd["compressor_inlet_pressure"]
                    _s_in = _CP.PropsSI("S", "H", _h_in, "P", _p_in, _fluid)
                    _q_out = _CP.PropsSI("Q", "S", _s_in, "P", _p_cond, _fluid)
                    _h_out_is = _CP.PropsSI("H", "S", _s_in, "P", _p_cond, _fluid)
                    _verdict = (
                        "⚠  CONFIRMED: isentropic outlet in two-phase dome"
                        if 0.0 <= _q_out <= 1.0
                        else "not in dome — different root cause"
                    )
                    print(
                        f"     CRASH-X:  p_in={_p_in/1e5:.4f} bar  "
                        f"h_in={_h_in:.0f} J/kg  p_cond={_p_cond/1e5:.4f} bar"
                    )
                    print(
                        f"     Isentropic outlet at (s_in, p_cond):  "
                        f"q={_q_out:.4f}   h_out_is={_h_out_is:.0f} J/kg   "
                        f"Dh_is_total={(_h_in-_h_out_is)/1e3:+.2f} kJ/kg"
                    )
                    print(f"     {_verdict}")
                except Exception as _dxe:
                    print(f"     [crash diagnostic skipped: {_dxe}]")
                result = {
    ...
    yaml_stem = Path(base_yaml).stem
    csv_path = output_dir / f"parametric_nstages_rpm_{yaml_stem}_{timestamp}.csv"
    xlsx_path = output_dir / f"parametric_nstages_rpm_{yaml_stem}_{timestamp}.xlsx"
...
def _pcond_fluid_worker(task):
    ...
    # TC is only physically meaningful when T_crit < heat-source max temperature.
    # For fluids where T_crit >= warmstart_tcrit_K the brine cannot supply
    # enough heat to reach supercritical conditions, so the TC attempt is
    # skipped entirely — it always fails and wastes ~40 s per fluid.
    tc_feasible = T_crit_fluid < warmstart_tcrit_K
    ...
    if dual_warmstart:
        ...
        if tc_feasible:
            ...
            row_tc = {
                "converged": False,
                "skip_reason": (
                    f"TC skipped: T_crit={T_crit_fluid-273.15:.1f}°C "
                    f">= heat-source limit {warmstart_tcrit_K-273.15:.1f}°C"
                ),
            }
        ...
        if sc_ok and tc_ok:
            ...
            row = row_sc
            row["warmstart_winner"] = "SC only" if not tc_feasible else "SC (TC failed)"
            ...
            row["skip_reason"] = (
                "SC warm start failed to converge"
                if not tc_feasible
                else "both SC and TC warm starts failed to converge"
            )
...
def _narrow_bounds(vd, pct=0.25, var_name="", fluid_name=None, T_max=475.30):
    """
...
def _multistart_sample_worker(args):
    ...
    Args tuple (all picklable primitives — no Path objects, no lambdas):
        ...
        bounds          : list[(lo, hi)]  — pre-computed narrow sampling bounds
                          (plain float tuples — expression bounds are resolved to
                          floats in the main process before workers are spawned,
                          so workers never receive expression strings)
    ...
    try:
        ...
        # ── Solver diagnostics ────────────────────────────────────────
        # pysolver_view exposes grad_count (= SLSQP gradient evaluations,
        # matching the "Grad-eval" column in the convergence table),
        # func_count_tot (total function evaluations), and
        # convergence_history["constraint_violation"] (infeasibility per iter).
        # The old attributes (nit, nfev, infeasibility) do not exist.
        try:
            row["n_iterations"] = int(cycle.solver.grad_count)
        except AttributeError:
            pass
        try:
            row["n_func_evals"] = int(cycle.solver.func_count_tot)
        except AttributeError:
            pass
        try:
            row["infeasibility"] = float(
                cycle.solver.convergence_history["constraint_violation"][-1]
            )
...
def run_multistart(config_file, n_samples, output_dir, fluid_override=None):
    ...
    # seed=42 makes results reproducible across runs.
    # optimization="random-cd" minimises centered L2-discrepancy, ensuring
    # samples are well-spread in the joint d-dimensional space, not just
    # marginally uniform in each dimension individually.
    sampler = LatinHypercube(d=n_vars, seed=42, optimization="random-cd")
    ...
    if optimized_yaml_path is not None:
        ...
        print()
        print("  Running full post-processing on best result ...")
        try:
            run_optimize(
                optimized_yaml_path,
                batch=True,
                dest_dir=output_dir / f"best_result_postprocessing_{timestamp}",
            )
            print(
                f"    Post-processing: {output_dir / f'best_result_postprocessing_{timestamp}'}"
            )
        except Exception as _pp_err:
            print(f"  Warning: post-processing failed: {_pp_err}")
    print("=" * 70 + "\n")
...
# ══════════════════════════════════════════════════════════════════════
#  PARAMETRIC STUDY — MODULE-LEVEL WORKER (must be at module level for
#  multiprocessing pickling on Windows)
# ══════════════════════════════════════════════════════════════════════


def _parametric_worker(base_yaml_str):
    """
    Run one full n_stages x RPM parametric sweep for a single base YAML.

    This is a module-level function so multiprocessing can pickle it on
    Windows (spawn start method).  Each worker runs its own independent
    sequential warm-start chain — parallelism is only across configurations,
    never within a single sweep.

    Parameters
    ----------
    base_yaml_str : str
        Absolute path to the base YAML for this configuration.

    Returns
    -------
    tuple (base_yaml_str, df) where df is the results DataFrame, or
    (base_yaml_str, None) if the sweep raised an unhandled exception.
    """
    import traceback as _tb

    base_yaml = Path(base_yaml_str)
    config_output_dir = PARAMETRIC_OUTPUT_DIR / base_yaml.stem
    try:
        df = run_parametric_nstages_rpm(
            base_yaml=base_yaml,
            n_stages_list=PARAMETRIC_N_STAGES_LIST,
            rpm_list=PARAMETRIC_RPM_LIST,
            rpm_to_n_turbines=PARAMETRIC_RPM_TO_N_TURBINES,
            rpm_turbines_list=PARAMETRIC_RPM_TURBINES_LIST,
        ...
        return base_yaml_str, df
    except Exception:
        print(f"\n  [ERROR] Parametric sweep failed for {base_yaml.name}:")
        _tb.print_exc()
        return base_yaml_str, None
...
if __name__ == "__main__":
    ...
    # ── Set up logging to save terminal output to a log file ──────────
    import logging as _logging
    import sys as _sys

    _log_dir = PARAMETRIC_OUTPUT_DIR
    _log_dir.mkdir(parents=True, exist_ok=True)
    _timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    _log_path = _log_dir / f"run_log_{MODE}_{_timestamp}.txt"
    ...
    print(f"Logging terminal output to: {_log_path}")
    print()
    ...
    if MODE == "optimize":
        ...
        # ── Merge all per-config Excels into one comparison file ──────
        comparison_path = batch_dir / "comparison_results.xlsx"
        print("=" * 70)
        print("  Merging per-config results into comparison_results.xlsx ...")
        print("=" * 70)
        _merge_batch_to_comparison_excel(
            batch_dir, BATCH_YAML_FILES, CONFIG_LABELS, comparison_path
        )
        ...
        if len(MULTISTART_YAML_FILES) == 1:
            ...
            for entry in MULTISTART_YAML_FILES:
                ...
                sampler = LatinHypercube(
                    d=len(var_names), seed=42, optimization="random-cd"
                )
            ...
            for (
                ...
                if optimized_yaml_path:
                    ...
                    print(f"  Running post-processing for best result of {label} ...")
                    try:
                        run_optimize(
                            optimized_yaml_path,
                            batch=True,
                            dest_dir=fluid_dir
                            / f"best_result_postprocessing_{timestamp}",
                        )
                        print(
                            f"     Post-proc  : best_result_postprocessing_{timestamp}/"
                        )
                    except Exception as _pp_err:
                        print(
                            f"  Warning: post-processing failed for {label}: {_pp_err}"
                        )
                summary_rows.append(
        ...
        # ── Validate all YAML files exist before starting ─────────────
        missing = [f for f in PARAMETRIC_BASE_YAMLS if not f.exists()]
        if missing:
            for f in missing:
                print(f"Error: base YAML not found: {f}")
            sys.exit(1)

        n_configs = len(PARAMETRIC_BASE_YAMLS)
        n_runs_each = len(PARAMETRIC_N_STAGES_LIST) * len(PARAMETRIC_RPM_LIST)

        print("=" * 76)
        print("  PARAMETRIC STUDY — BATCH LAUNCH")
        print("=" * 76)
        print(f"  Configurations : {n_configs}")
        print(
            f"  Runs each      : {n_runs_each}  "
            f"({len(PARAMETRIC_N_STAGES_LIST)} n_stages "
            f"x {len(PARAMETRIC_RPM_LIST)} RPM values)"
        )
        print(f"  Total runs     : {n_configs * n_runs_each}")
        print(f"  Output root    : {PARAMETRIC_OUTPUT_DIR}")
        for f in PARAMETRIC_BASE_YAMLS:
            print(f"    - {f.name}")
        print("=" * 76)

        if n_configs == 1:
            # ── Single config: run directly in main process ───────────
            print("  (single config — running sequentially in main process)")
            print()
            _, df = _parametric_worker(str(PARAMETRIC_BASE_YAMLS[0]))

        else:
            # ── Multiple configs: one worker process per config ───────
            actual_workers = min(PARAMETRIC_N_WORKERS, n_configs, _mp.cpu_count())
            print(
                f"  Workers        : {actual_workers} "
                f"(of {_mp.cpu_count()} CPUs, "
                f"{n_configs} configs)"
            )
            print()

            yaml_strs = [str(f) for f in PARAMETRIC_BASE_YAMLS]

            all_dfs = {}
            with _mp.Pool(processes=actual_workers) as pool:
                for yaml_str, df in pool.imap_unordered(_parametric_worker, yaml_strs):
                    name = Path(yaml_str).stem
                    all_dfs[name] = df
                    status = (
                        f"✓  {len(df)} rows saved"
                        if df is not None
                        else "✗  failed (see traceback above)"
                    )
                    print(f"  [{name}]  {status}")
            ...
            print()
            print("=" * 76)
            print("  PARAMETRIC STUDY — FINAL SUMMARY")
            print("=" * 76)
            for yaml_file in PARAMETRIC_BASE_YAMLS:
                name = yaml_file.stem
                df = all_dfs.get(name)
                if df is None:
                    print(f"  {name:<50}  ✗  sweep failed")
                    continue
                n_ok = int(df["converged"].sum()) if "converged" in df.columns else "?"
                n_total = len(df)
                best_eta = (
                    df["eta_system"].max() * 100
                    if "eta_system" in df.columns and df["eta_system"].notna().any()
                    else None
                )
                eta_str = (
                    f"best η_sys={best_eta:.2f}%" if best_eta else "no converged runs"
                )
                print(f"  {name:<50}  {n_ok}/{n_total} converged  {eta_str}")
            print("=" * 76)
