...
def classify_binding(row: pd.Series, tol: float = 1e-3) -> str:
    """For one run, determine which Macchi envelope constraint binds most tightly.
    Returns a short code: 'Ns_hi', 'Ns_lo', 'Vr_hi', 'Vr_lo', or '—'."""
    n = int(row["n_stages"])
    ...
    slacks = []
    if ns_vals:
        slacks.append(("Ns_lo", min(ns_vals) - NS_LO))
        slacks.append(("Ns_hi", NS_HI - max(ns_vals)))
    if vr_vals:
        slacks.append(("Vr_lo", min(vr_vals) - VR_LO))
        slacks.append(("Vr_hi", VR_HI - max(vr_vals)))

    if not slacks:
        return "—"
    code, slack = min(slacks, key=lambda t: t[1])
    return code if slack < tol else "—"


def plot_fig2_constraints(df: pd.DataFrame, fluid: str, outdir: Path) -> None:
    df = df.copy()
    df["binding"] = df.apply(classify_binding, axis=1)

    n_stages_list = sorted(df["n_stages"].unique())
    combos = sorted(df["combo"].unique())

    # Build a matrix: rows = (RPM, n_turb), cols = n_stages
    # Cell value = binding constraint code
    label_map = {
        "Ns_hi": r"$N_s=0.195$",
        "Ns_lo": r"$N_s=0.045$",
        "Vr_hi": r"$V_r=5.0$",
    ...
    for i, combo in enumerate(combos):
        for j, n in enumerate(n_stages_list):
            ...
            if row.empty:
                continue
            code = row["binding"].iloc[0]
            eta = row["eta_system"].iloc[0] * 100
            ax.add_patch(plt.Rectangle(
                (j - 0.48, i - 0.48), 0.96, 0.96,
                facecolor=code_color[code], edgecolor="white", linewidth=1.5,
            ))
            ax.text(j, i + 0.14, label_map[code],
    ...
    ax.set_title(f"{fluid}: binding Macchi-Astolfi constraint at each converged run",
                 fontsize=11)
    ax.invert_yaxis()
    ax.set_aspect("equal")
    ...
    ax.legend(handles, labels, loc="center left", bbox_to_anchor=(1.02, 0.5),
              fontsize=9, frameon=False, title="Binding constraint")
