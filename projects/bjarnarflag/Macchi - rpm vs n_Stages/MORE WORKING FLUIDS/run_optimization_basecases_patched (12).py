...
# Prefix used for intermediate-stage design variable names.
# Using the RATIO parameterization: expander_intermediate_ratio_i ∈ (0, 1).
# Thermopt reconstructs absolute pressures internally via
#   p_i = p_out * (p_{i-1} / p_out) ** r_i,   p_0 = p_in
# which guarantees p_in > p_1 > ... > p_out by construction and
# eliminates the stage-inversion NaN crashes in SLSQP's finite-diff
# gradient.
# REQUIRES the matching thermopt patch in cycle_power_recuperated.py.
PARAMETRIC_INTERP_PREFIX = "expander_intermediate_ratio"

# Bounds on each ratio. Pulled off 0 and 1 so SLSQP can't pin a stage
# at a zero-expansion endpoint.
PARAMETRIC_INTERP_P_MIN_EXPR = 0.01
PARAMETRIC_INTERP_P_MAX_EXPR = 0.99
...
def run_parametric_nstages_rpm(
    ...
    for n_stages in n_stages_list:
        ...
        non_ordering = [
            ...
            and not _is_ordering_constraint(c, "expander_intermediate_pressure")
        ]

        # With the ratio parameterization, the physical ordering
        #     p_in > p_1 > ... > p_{n-1} > p_out
        # is guaranteed by construction (each r_i in (0, 1) produces a
        # pressure strictly between its neighbors). Adding explicit
        # ordering constraints would compare a dimensionless ratio
        # against an absolute pressure — a meaningless expression that
        # SLSQP reports as persistent infeasibility.
        cycle.config["problem_formulation"]["constraints"] = non_ordering
        print(
            f"    Ordering: guaranteed by ratio parameterization "
            f"(p_in > p_1 > ... > p_out by construction)"
        )
        ...
        # ── 4b. Initialize intermediate-stage ratios ──────────────────
        # Equal-ratio initial guess in the nested log form:
        #     r_i = (n - i) / (n - i + 1),    for i = 1..n-1
        # gives equal geometric pressure ratios per stage. The physical
        # pressure each ratio reconstructs to is printed alongside for
        # sanity checking; thermopt does the reconstruction internally
        # on every evaluation.
        ...
        if p_display_ok:
            print(
                f"    Warmstart reference pressures: "
                f"p_in={p_in/1e5:.4f} bar, p_out={p_out/1e5:.4f} bar"
            )

        p_prev = p_in if p_display_ok else None
        for idx, var_name in enumerate(existing, 1):
            r_guess = float((n_stages - idx) / (n_stages - idx + 1))
            dvars[var_name]["value"] = r_guess
            dvars[var_name]["min"] = interp_p_min_expr
            dvars[var_name]["max"] = interp_p_max_expr
            cycle.set_config_value(
            ...
            if p_display_ok:
                p_reconstructed = p_out * (p_prev / p_out) ** r_guess
                print(
                    f"    ↺ set {var_name} = {r_guess:.4f}   "
                    f"bounds [{interp_p_min_expr}, {interp_p_max_expr}]   "
                    f"(⇒ p_{idx} = {p_reconstructed/1e5:.4f} bar "
                    f"at current p_in, p_out)"
                )
                p_prev = p_reconstructed
            else:
                print(
                    f"    ↺ set {var_name} = {r_guess:.4f}   "
                    f"bounds [{interp_p_min_expr}, {interp_p_max_expr}]"
