...
def evaluate_cycle(
    ...
    """
    Dual-pressure ORC cycle with two separate brine sources at different
    pressure and temperature levels.

    The HP brine feeds only the HP evaporator and is then reinjected.
    The LP brine feeds the LP evaporator and the preheater in series
    before being reinjected by a separate pump.

    Both brine mass flow rates (``mass_flow_rate`` under ``heat_source_hp``
    and ``heat_source_lp``) must be provided as fixed parameters or design
    variables. The working fluid cycle mass flow (``mass_flow_rate_cycle``)
    and cooling fluid mass flow (``mass_flow_rate_sink``) are design variables.
    Heat exchanger energy balances are automatically enforced as equality
    constraints inside the heat exchanger model.
    """
    ...
    # Extract HP and LP brine source parameters
    T_source_hp_out_min = parameters["heat_source_hp"].pop("minimum_temperature")
    p_source_hp_out = parameters["heat_source_hp"].pop("exit_pressure")
    source_hp_out_min = heating_fluid.get_state(
        cpx.PT_INPUTS, p_source_hp_out, T_source_hp_out_min
    )

    T_source_lp_out_min = parameters["heat_source_lp"].pop("minimum_temperature")
    p_source_lp_out = parameters["heat_source_lp"].pop("exit_pressure")
    source_lp_out_min = heating_fluid.get_state(
        cpx.PT_INPUTS, p_source_lp_out, T_source_lp_out_min
    )

    # Total brine mass flow and fixed well fraction
    hp_well_fraction = parameters.pop("hp_well_fraction")
    m_brine_total = variables.pop("well_mass_flow_rate_total")
    m_brine_hp = hp_well_fraction * m_brine_total
    m_brine_lp = (1.0 - hp_well_fraction) * m_brine_total

    # Ambient reference states for each brine stream (for η_utilization / η_ambient)
    ...
    source_hp_out_ambient = heating_fluid.get_state(cpx.PT_INPUTS, p_ambient, T_ambient)
    source_lp_out_ambient = source_hp_out_ambient  # same dead state for both streams
    T0 = T_ambient
    p0 = p_ambient
    ...
    p_in_hot_brine_hp = parameters["heat_source_hp"].pop("inlet_pressure")
    if "inlet_enthalpy" in parameters["heat_source_hp"]:
        h_in_hot_brine_hp = parameters["heat_source_hp"].pop("inlet_enthalpy")
    else:
        T_in_hot_brine_hp = parameters["heat_source_hp"].pop("inlet_temperature")
        h_in_hot_brine_hp = heating_fluid.get_state(
            cpx.PT_INPUTS, p_in_hot_brine_hp, T_in_hot_brine_hp
        ).h

    p_in_hot_brine_lp = parameters["heat_source_lp"].pop("inlet_pressure")
    h_in_hot_brine_lp = parameters["heat_source_lp"].pop("inlet_enthalpy")

    # Compute brine exit temperatures from fractions — ordering guaranteed by construction:
    #   HP stream: T_reinjection_hp <= T_hp_evap_exit <= T_hp_brine_inlet
    #   LP stream: T_reinjection_lp <= T_preheater_exit <= T_lp_evap_exit <= T_lp_brine_inlet
    T_source_hp_in = heating_fluid.get_state(
        cpx.HmassP_INPUTS, h_in_hot_brine_hp, p_in_hot_brine_hp
    ).T
    T_source_lp_in = heating_fluid.get_state(
        cpx.HmassP_INPUTS, h_in_hot_brine_lp, p_in_hot_brine_lp
    ).T
    heat_source_hp_evap_exit_temp = (
        T_source_hp_out_min
        + heat_source_hp_evap_exit_fraction * (T_source_hp_in - T_source_hp_out_min)
    )
    heat_source_lp_evap_exit_temp = (
        T_source_lp_out_min
        + heat_source_lp_evap_exit_fraction * (T_source_lp_in - T_source_lp_out_min)
    )
    heat_source_preheater_exit_temp = (
        T_source_lp_out_min
        + heat_source_preheater_exit_fraction
        * (heat_source_lp_evap_exit_temp - T_source_lp_out_min)
    )

    p_out_hot_hp = p_in_hot_brine_hp * (1.0 - dp_hp_evap_h)
    ...
    h_out_hot_pre = heating_fluid.get_state(
        cpx.PT_INPUTS,
        p_in_hot_pre * (1.0 - dp_preheater_h),
        heat_source_preheater_exit_temp,
    ).h
    ...
    hp_brine_exit_temperature = hp_evaporator["hot_side"]["state_out"].T
    lp_brine_exit_temperature = preheater["hot_side"]["state_out"].T
    brine_exit_temperature = min(hp_brine_exit_temperature, lp_brine_exit_temperature)

    # Evaluate HP brine reinjection pump
    eff_hs_hp = parameters["heat_source_pump_hp"].pop("efficiency")
    eff_type_hs_hp = parameters["heat_source_pump_hp"].pop("efficiency_type")
    hs_pump_hp = compression_process(
        heating_fluid,
        hp_evaporator["hot_side"]["state_out"].h,
        hp_evaporator["hot_side"]["state_out"].p,
        p_source_hp_out,
        eff_hs_hp,
        eff_type_hs_hp,
        mass_flow=m_brine_hp,
        T0=T0,
        p0=p0,
    )

    # Evaluate LP brine reinjection pump
    eff_hs_lp = parameters["heat_source_pump_lp"].pop("efficiency")
    eff_type_hs_lp = parameters["heat_source_pump_lp"].pop("efficiency_type")
    hs_pump_lp = compression_process(
        heating_fluid,
    ...
    components = {
        ...
        "cooler": cooler,
        "heat_source_pump_hp": hs_pump_hp,
        "heat_source_pump_lp": hs_pump_lp,
    ...
    W_aux = (
        hs_pump_hp["energy_analysis"]["power"]
        + hs_pump_lp["energy_analysis"]["power"]
        + heat_sink_pump["energy_analysis"]["power"]
    )
    W_in = W_comp + W_aux
    W_net = W_out - W_comp  # net cycle power (turbines minus WF pumps)

    Q_in_max = m_brine_hp * (h_in_hot_brine_hp - source_hp_out_min.h) + m_brine_lp * (
        h_in_hot_brine_lp - source_lp_out_min.h
    )
    Q_in_max_ambient = m_brine_hp * (
        h_in_hot_brine_hp - source_hp_out_ambient.h
    ) + m_brine_lp * (h_in_hot_brine_lp - source_lp_out_ambient.h)
    ...
    energy_analysis = {
        ...
        "mass_flow_brine_hp": m_brine_hp,
        "mass_flow_brine_lp": m_brine_lp,
        ...
        "hp_brine_exit_temperature": hp_brine_exit_temperature,
        "lp_brine_exit_temperature": lp_brine_exit_temperature,
    }
