...
def evaluate_cycle(
    ...
    if "mass_flow_rate" in parameters["heat_source_hp"]:
        m_brine_hp = parameters["heat_source_hp"].pop("mass_flow_rate")
    else:
        m_brine_hp = variables.pop("mass_flow_rate_hp")
    ...
    if "mass_flow_rate" in parameters["heat_source_lp"]:
        m_brine_lp = parameters["heat_source_lp"].pop("mass_flow_rate")
    else:
        m_brine_lp = variables.pop("mass_flow_rate_lp")
    ...
    # HP brine feeds only the HP evaporator.
    # ------------------------------------------------------------------
    ...
    # LP brine is a completely separate stream from the HP brine.
    # ------------------------------------------------------------------
    h_in_cold_lp = h_3
    p_in_cold_lp = p_3
    ...
    h_out_hot_lp = h_in_hot_brine_lp - (m_LP / m_brine_lp) * (h_out_cold_lp - h_in_cold_lp)
    ...
    # ------------------------------------------------------------------
    # Evaluate preheater
    # LP brine continues from the LP evaporator exit into the preheater.
    # ------------------------------------------------------------------
    ...
    h_out_hot_pre = h_in_hot_pre - (m_total / m_brine_lp) * (h_out_cold_pre - h_in_cold_pre)
    ...
    hs_pump_hp["mass_flow"] = m_brine_hp
    hs_pump_lp["mass_flow"] = m_brine_lp
    ...
    W_aux = hs_pump_hp["power"] + hs_pump_lp["power"] + heat_sink_pump["power"]
    W_in = W_comp + W_aux
    Q_in_max = (
        m_brine_hp * (h_in_hot_brine_hp - source_hp_out_min.h)
        + m_brine_lp * (h_in_hot_brine_lp - source_lp_out_min.h)
    )
    Q_in_max_ambient = (
        m_brine_hp * (h_in_hot_brine_hp - source_hp_out_ambient.h)
    ...
    energy_analysis = {
        ...
        "heat_source_pump_hp_power": hs_pump_hp["power"],
        "heat_source_pump_lp_power": hs_pump_lp["power"],
